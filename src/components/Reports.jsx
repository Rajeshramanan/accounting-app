...
const Reports = ({ view, vouchers, ledgers, stock }) => {
        ...
        <div className="flex-1 overflow-auto border border-gray-500 dark:border-accounting-dark-border bg-white dark:bg-accounting-dark-panel">
            <div className="min-w-max">
                <table className="w-full border-collapse">
                    ...
                    <tbody>
                        {vouchers.length === 0 ? (<tr><TableCell className="text-center italic text-gray-500 dark:text-accounting-dark-muted py-4" align="left">-- No Records Found --</TableCell></tr>) : (vouchers.map((v, idx) => {
            const drEntry = v.entries.find(e => e.type === 'Dr');
            return (<tr key={v.id} className={idx % 2 === 0 ? 'bg-white dark:bg-accounting-dark-panel' : 'bg-[#f9f9f9] dark:bg-[#252525]'}>
        ...
        <div className="flex-1 overflow-auto border border-gray-500 dark:border-accounting-dark-border bg-white dark:bg-accounting-dark-panel">
            <div className="min-w-max">
                <table className="w-full border-collapse">
                    ...
                    <tbody>
                        {stock.map((item, idx) => (<tr key={item.id} className={idx % 2 === 0 ? 'bg-white dark:bg-accounting-dark-panel' : 'bg-[#f9f9f9] dark:bg-[#252525]'}>
    ...
    const LedgerSummary = () => (<div className="p-2 h-full overflow-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
             {['Capital', 'Current Assets', 'Current Liabilities', 'Income', 'Expenses', 'Tax'].map(group => (<div key={group} className="border border-gray-400 dark:border-accounting-dark-border bg-white dark:bg-accounting-dark-panel">
                     ...
                     <div>
                        {ledgers.filter(l => l.group === group).map((l, idx) => (<div key={l.id} className={`flex justify-between text-xs px-2 py-1 border-b border-gray-300 dark:border-accounting-dark-border last:border-0 ${idx % 2 !== 0 ? 'bg-[#f9f9f9] dark:bg-[#252525]' : 'dark:bg-accounting-dark-panel'}`}>
    ...
    const FinancialReports = () => {
        ...
        return (<div className="p-2 h-full flex flex-col overflow-auto">
          <div className="mb-2 text-xs font-bold text-black dark:text-accounting-dark-text uppercase border-b border-black dark:border-accounting-dark-border pb-1 shrink-0">
          ...
          <div className="flex flex-col md:flex-row text-xs border border-gray-500 dark:border-accounting-dark-border bg-white dark:bg-accounting-dark-panel min-w-max md:min-w-0">
              ...
              <div className="flex-1 md:border-r border-gray-500 dark:border-accounting-dark-border border-b md:border-b-0">
                  ...
                  <div>
                      ...
                      {expenses.map(l => (<div key={l.id} className="flex justify-between px-2 py-1 border-b border-gray-200 dark:border-accounting-dark-border">
                      ...
                      {netDiff > 0 && (<div className="flex justify-between px-2 py-1 font-bold mt-4 bg-[#f0f0f0] dark:bg-[#252525] border-t border-gray-400 dark:border-accounting-dark-border text-black dark:text-accounting-dark-text">
